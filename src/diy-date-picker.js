...
;(function (factory) {
  ...
  if (typeof module !== 'undefined' && typeof exports !== 'undefined' && !exports.nodeType) {
    ...
    // register diy-date-picker
    globalObj.diyDatePicker = factory();
    // if exist amd
    if (typeof define === 'function' && define.amd) {
      define([], function () {
        return globalObj.diyDatePicker;
      });
    }
  }
}(function () {
  ...
  function getElements (selectors) {
    ...
    if (typeof selectors === 'string') { // strings selector
      ...
      throw new Error('Invalid selector for diy date picker!');
    }
  ...
  DatePicker.prototype.initDom = function () {
    ...
    var $view = [
      '<div class="diy-date-picker__header">',
      '<button class="diy-date-picker__btn diy-date-picker__prev-btn diy-date-picker__prev-year">' + prevYearArrow + '</button>',
      '<button class="diy-date-picker__btn diy-date-picker__prev-btn diy-date-picker__prev-month">' + prevMonthArrow + '</button>',
      '<span class="diy-date-picker__header-label diy-date-picker__show-year"></span>',
      '<span class="diy-date-picker__header-label diy-date-picker__show-month"></span>',
      '<button class="diy-date-picker__btn diy-date-picker__next-btn diy-date-picker__next-year">' + nextYearArrow + '</button>',
      '<button class="diy-date-picker__btn diy-date-picker__next-btn diy-date-picker__next-month">' + nextMonthArrow + '</button>',
      '</div>',
      '<div class="diy-date-picker__body">',
      '<table class="diy-date-picker__table diy-date-picker__date-table">',
      '<thead></thead>',
      '<tbody></tbody>',
      '</table>',
      '<table class="diy-date-picker__table diy-date-picker__month-table">',
      '<tbody></tbody>',
      '</table>',
      '<table class="diy-date-picker__table diy-date-picker__year-table">',
      '<tbody></tbody>',
      '</table>',
      '</div>',
      '<div class="diy-date-picker__footer" style="display: ' + (clearable ? 'block' : 'none') + '">',
      '<button class="diy-date-picker__footer-btn diy-date-picker__now-btn"><span>' + todayText + '</span></button>',
      '<button class="diy-date-picker__footer-btn diy-date-picker__clear-btn"><span>' + clearText + '</span></button>',
      '</div>'];
    ...
    $datePicker.className = 'diy-date-picker__wrapper';
    $datePicker.innerHTML = $view.join('');

    // init dom for datepicker
    this.$datePicker = $datePicker;
    this.$showYear = this.getDom('.diy-date-picker__show-year');
    this.$showMonth = this.getDom('.diy-date-picker__show-month');
    this.$dateTable = this.getDom('.diy-date-picker__date-table');
    this.$monthTable = this.getDom('.diy-date-picker__month-table');
    this.$yearTable = this.getDom('.diy-date-picker__year-table');
    this.$prevYear = this.getDom('.diy-date-picker__prev-year');
    this.$prevMonth = this.getDom('.diy-date-picker__prev-month');
    this.$nextMonth = this.getDom('.diy-date-picker__next-month');
    this.$nextYear = this.getDom('.diy-date-picker__next-year');
    this.$datePickerFooter = this.getDom('.diy-date-picker__footer');
    this.$nowBtn = this.getDom('.diy-date-picker__now-btn');
    this.$clearBtn = this.getDom('.diy-date-picker__clear-btn');
  };
  ...
  DatePicker.prototype.adjustPosition = function () {
    ...
    var className = 'diy-date-picker__wrapper diy-date-picker__';
    ...
    if (this.settings.clearable) {
      className += ' diy-date-picker__clearable';
    }
  ...
  DatePicker.prototype.getI18n = function () {
    var i18n = diyDatePicker.langs[this.settings.lang || 'zh-CN'];
    if (!i18n) {
  ...
  // init
  diyDatePicker.init = function (selectors, options) {
  ...
  // proxy
  diyDatePicker.proxy = function (selectors, methods) {
    var elements = getElements(selectors);
  ...
  // destroy
  diyDatePicker.destroy = function (selectors) {
    this.proxy(selectors, 'destroy');
  };

  // show
  diyDatePicker.show = function (selectors) {
    this.proxy(selectors, 'onShow');
  };

  // hide
  diyDatePicker.hide = function (selectors) {
    this.proxy(selectors, 'hide');
  };

  // setDate
  diyDatePicker.setDate = function (selectors, newDate) {
    this.proxy(selectors, 'setDate', newDate);
  };

  // getDate
  diyDatePicker.getDate = function (selectors) {
    return this.proxy(selectors, 'getDate');
  };

  // clearDate
  diyDatePicker.clearDate = function (selectors) {
    this.proxy(selectors, 'clearDate');
  };

  return diyDatePicker;
}));
